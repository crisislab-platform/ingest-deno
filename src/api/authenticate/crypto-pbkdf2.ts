 ...
 * @param   {Number} [iterations=1e5] - Number of iterations of HMAC function to apply.
...
export async function pbkdf2(
	...
	const doubleDots = iterHex.match(/.{2}/g);
	if (!doubleDots) throw "Didn't find double dots";
	const iterArray = doubleDots.map((byte) => parseInt(byte, 16)); // iter’ns as byte array
...
export async function pbkdf2Verify(key: string, password: string) {
	...
	const sections = saltStr.match(/./g);
	if (!sections) throw "Invalid salt str";
	const saltUint8 = new Uint8Array(sections.map((ch) => ch.charCodeAt(0))); // salt as Uint8Array
	...
	if (!iterHex) throw "Invalid iterHex";
